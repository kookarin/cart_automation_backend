...
// Function to extract the first 5 products from the search API response
function getFirstFiveProducts(searchResult) {
    // in the search result there is a layout object, it has array of objects if that object has widgetId "PRODUCT_GRID"  then we can get the items from the data.resolver.data.items
    const allProducts = [];
...
function transformProducts(products, itemName) {
    return {
...
async function collectGroceryProducts(groceryList) {
    const results = {};
...
// Example usage with a list of grocery items
const groceryList = ["eggoz eggs", "amul milk", "bread", "rice", "dal"];

// save results to a file
import fs from 'fs';
const results = await collectGroceryProducts(groceryList);
fs.writeFileSync('zepto_products.json', JSON.stringify(results, null, 2));
